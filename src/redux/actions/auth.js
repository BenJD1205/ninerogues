	...
	(first_name, last_name, email, password, re_password) => async (dispatch) => {
		...
		try {
			...
			if (res.status === 201) {
				...
				dispatch(setAlert("Đăng kí tài khoản thành công", "green"));
			} else {
				dispatch({
					type: SINGUP_FAIL,
				});
				dispatch(setAlert("Đăng kí tài khoản thất bại", "red"));
			}
			dispatch({
...
export const activate = (uid, token) => async (dispatch) => {
	...
	try {
		...
		if (res.status === 204) {
			...
			dispatch(setAlert("Active tài khoản thành công", "green"));
		} else {
			dispatch({
				type: ACTIVATION_FAIL,
			});
			dispatch(setAlert("Active tài khoản thất bại", "red"));
		}
		dispatch({
			type: REMOVE_AUTH_LOADING,
		});
...
export const login = (email, password) => async (dispatch) => {
	...
	try {
		...
		if (res.status === 200) {
			...
			dispatch(setAlert("Đăng nhập thành công", "green"));
		} else {
			dispatch({
				type: LOGIN_FAIL,
			});
			...
			dispatch(setAlert("Đăng nhập thất bại", "red"));
		}
	} catch (err) {
		dispatch({
			type: ACTIVATION_FAIL,
...
export const logout = () => async (dispatch) => {
	...
	dispatch(setAlert("Đăng xuất thành công", "green"));
};
